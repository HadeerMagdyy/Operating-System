...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	//panic("create_page_table() is not implemented yet...!!");
	    ...
	    uint32* address = kmalloc(PAGE_SIZE) ;
	   if(address==0){
		   return 0;
	   }
	   else{
		ptr_page_directory[PDX(virtual_address)] =kheap_physical_address((uint32)address)|PERM_WRITEABLE| PERM_PRESENT | PERM_USER;
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for (uint32 i=virtual_address; i < virtual_address + size; i += PAGE_SIZE) {
		pf_add_empty_env_page(e,i | PERM_WRITEABLE | PERM_USER | PERM_PRESENT, 0);
	}
...
{
	/*uint32 *ptr=NULL;
	uint32 size2=ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
for (uint32 i=virtual_address ;i<virtual_address+size;i+=PAGE_SIZE){

	 uint32 permissions = pt_get_page_permissions(e, i);
//2
	 if((permissions&PERM_BUFFERED)){
		  struct Frame_Info *frame= get_frame_info(e->env_page_directory, (void*)i, &ptr);
...
//3
		for (int j = 0; j < e->page_WS_max_size; j++) {
					...
					uint32 *table=NULL;
				  //  struct Frame_Info *f= get_frame_info(e->env_page_directory, (void*)va, &table);
						if (va == i) {
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 *ptr=NULL;
		uint32 size2=ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
	for (uint32 i=virtual_address ;i<size2;i+=PAGE_SIZE){
	...
	//3
			...
			if (ptr != NULL)
				...
				z=1;
						...
						if (z)
						{
							unmap_frame(e->env_page_directory,(void *)ptr);
							pd_clear_page_dir_entry(e, i);
						}
					}
			pf_remove_env_page(e, i);
