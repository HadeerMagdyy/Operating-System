...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	uint32 workingset_size=env_page_ws_get_size(curenv);
	uint32 page_permissions = pt_get_page_permissions(curenv, fault_va);
	int found =0;

	if(workingset_size >=curenv->page_WS_max_size){

		uint32 elvictim;
        try1 :
		for(unsigned int i=0;i<curenv->page_WS_max_size;i++){

			uint32 virt=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
			uint32 page_permission =pt_get_page_permissions(curenv, virt);
			if((page_permission&PERM_USED)==0&& (page_permission&PERM_MODIFIED)==0)
			{
		        elvictim=virt;
				found++;
				break;
	...
	curenv->page_last_WS_index=(curenv->page_last_WS_index+1)%curenv->page_WS_max_size;
		...
		for(unsigned int j=0;j<curenv->page_WS_max_size;j++){

			uint32 virt=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
			uint32 page_permission =pt_get_page_permissions(curenv, virt);
			if((page_permission&PERM_USED)==0){

				elvictim=virt;
				found++;
				break;
					}
			//fl str ely t7t daa s7 wla h3ml mkan l last ws index el j?
					pt_set_page_permissions(curenv, virt, 0, PERM_USED);
                curenv->page_last_WS_index=(curenv->page_last_WS_index+1)%curenv->page_WS_max_size;
				}
		if(found==0){
				goto try1;
		...
		{
		uint32 *ptr;
		struct Frame_Info *ptr_victim_frame = get_frame_info(curenv->env_page_directory, (void*)elvictim, &ptr);
		ptr_victim_frame->isBuffered=1;
		ptr_victim_frame->environment=curenv;
		ptr_victim_frame->va=elvictim;
		pt_set_page_permissions(curenv, elvictim, PERM_BUFFERED,PERM_PRESENT);
		uint32 page_permission = pt_get_page_permissions(curenv, elvictim);

		if((page_permission&PERM_MODIFIED)==0){
			...
			bufferList_add_page(&modified_frame_list, ptr_victim_frame);
					...
					if(size>=max_len){
						LIST_FOREACH(ptr_victim_frame, &modified_frame_list)
						{

                       pf_update_env_page(ptr_victim_frame->environment,(void*)ptr_victim_frame->va, ptr_victim_frame);
                       bufferlist_remove_page(&modified_frame_list,ptr_victim_frame);
                       pt_set_page_permissions(ptr_victim_frame->environment,ptr_victim_frame->va, 0, PERM_MODIFIED);
                       bufferList_add_page(&free_frame_list, ptr_victim_frame);
...
//placement
	if((page_permissions&PERM_BUFFERED)!=0){

				//1- set persent bit to 1 and buffered bit 0
				pt_set_page_permissions(curenv, fault_va, PERM_PRESENT, PERM_BUFFERED);

				uint32 *ptr;
				struct Frame_Info *ptr_frame_info = get_frame_info(curenv->env_page_directory, (void*)fault_va, &ptr);
				//Updates an existing page in the page file by the given frame in memory
				pf_update_env_page(curenv, (void*)fault_va, ptr_frame_info);
	...
	//lw hya msh buffered
					...
					if(ret == E_PAGE_NOT_EXIST_IN_PF) //page msh mwgoda 3l page file fa hashof hya fl range stack wla la
					{
						 if((fault_va > USTACKBOTTOM ) && (fault_va < USTACKTOP)){
								int rett=pf_add_empty_env_page(curenv, fault_va, 1);
						...
						{
							panic("illegal access to page");
						}

					}

					//update workingset by set Virtual Address of Page in Working Set
	...
	env_page_ws_set_entry(curenv, curenv->page_last_WS_index,fault_va);
	 if(curenv->page_last_WS_index < (curenv->page_WS_max_size-1))
					 ...
					 else if(curenv->page_last_WS_index == curenv->page_WS_max_size){
