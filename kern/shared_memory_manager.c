...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	if (count_created_objects > MAX_SHARES)
	{
		return E_NO_SHARE;
	}
	struct Share*ptr_share = &shares[ownerID];
	int id = allocate_share_object(&ptr_share);
	int frames_needed = size/PAGE_SIZE;
	if (size % PAGE_SIZE > 0)
		frames_needed++;
	struct Frame_Info*ptr_info = 0;
	for(int i =0;i<frames_needed;i++)
	{
		allocate_frame(&ptr_info);
		map_frame(myenv->env_page_directory,ptr_info,virtual_address,PERM_USER | PERM_WRITEABLE|PERM_PRESENT);
	...
	for(int i=0;i<frames_needed;i++)
	{
		add_frame_to_storage(shares[id].framesStorage,ptr_info,i);
	}

	//if (id == E_SHARED_MEM_EXISTS)
	//change this "return" according to your answer
	count_created_objects++;
	return id;
}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	{
		int frames_number = shares[id].size/PAGE_SIZE;
		if (shares[id].size % PAGE_SIZE > 0)
			frames_number++;
		...
		if (shares[id].isWritable == 1)
		{
			for(int i = 0;i<frames_number;i++)
				{
				struct Frame_Info* ptr_info = get_frame_from_storage(shares[id].framesStorage,i);
			    map_frame(myenv->env_page_directory,ptr_info,(void*)virtual_address,PERM_PRESENT|PERM_WRITEABLE|PERM_USER);
			    virtual_address+=PAGE_SIZE;
		...
		else
		{
			...
			{
			struct Frame_Info* ptr_info = get_frame_from_storage(shares[id].framesStorage,i);
			map_frame(myenv->env_page_directory,ptr_info,(void*)virtual_address,PERM_PRESENT|PERM_USER);
			virtual_address+=PAGE_SIZE;
			shares[id].references++;
			}
		}
