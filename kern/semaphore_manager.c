...
{
	int r= get_semaphore_object_ID(ownerEnvID,semaphoreName );
	if (r==E_SEMAPHORE_NOT_EXISTS)

	{
	 struct Semaphore*s;
	 int ret= allocate_semaphore_object(&s);
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int r= get_semaphore_object_ID(ownerEnvID, semaphoreName);
	semaphores[r].value-=1;
	if (semaphores[r].value<0)
	{ sched_remove_ready(myenv);
	 enqueue(&semaphores[r].env_queue,myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	// your code is here, remove the panic and write your code
		...
		if (semaphores[r].value<=0)
		{
		  struct Env* myenv = dequeue(&semaphores[r].env_queue);
